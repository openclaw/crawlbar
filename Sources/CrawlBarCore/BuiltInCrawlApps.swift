...
public enum BuiltInCrawlApps {
    ...
    public static let birdclaw = CrawlAppManifest(
        id: Self.birdclawID,
        displayName: "X",
        description: "Local-first X/Twitter archive and workspace",
        ...
        commands: [
            ...
            "doctor": ["auth", "status", "--json"],
            "search": ["--json", "search", "tweets"],
        ],
        capabilities: [.status, .doctor, .search],
        privacy: .init(containsPrivateMessages: true, exportsSecrets: false, localOnlyScopes: ["X archive", "DMs", "browser cookies"]),
        configOptions: [
            .init(id: "execution_mode", label: "Run location", kind: .choice, help: "Run birdclaw on this Mac or over SSH on another machine.", defaultValue: "local", choices: ["local", "remote"]),
            .init(id: "remote_target", label: "SSH target", help: "SSH target that can run birdclaw.", placeholder: "user@example-host"),
            .init(id: "remote_run_as", label: "Run as user", help: "Optional remote Unix user for sudo -u, when birdclaw is installed under a service account.", placeholder: "crawl"),
        ],
        configSections: [
            .init(id: "execution", title: "Execution", optionIDs: ["execution_mode"]),
            .init(id: "remote", title: "Remote Host", optionIDs: ["remote_target", "remote_run_as"]),
